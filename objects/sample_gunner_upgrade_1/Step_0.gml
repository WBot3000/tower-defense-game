...
if(shot_timer >= _frames_per_shot && ds_list_size(enemies_in_range) > 0) { //More than seconds_per_shot have ellapsed since last shot, so you can shoot again
	...
	var _bullet = instance_create_layer(x + sprite_width/2, y + sprite_height/2, PROJECTILE_LAYER, sample_gunner_upgrade_1_bullet,
		{
			//Need an initial speed in the event the target is destroyed right away. Otherwise, the missile would stall.
			x_speed: _vector_x * other.shooting_speed,
			y_speed: _vector_y * other.shooting_speed,
			missile_speed: other.shooting_speed,

...
var _toggle_button_pressed = keyboard_check_pressed(ord(global.GAME_CONFIG_OPTIONS.controls.open_purchase_menu_key));
...
switch (state) {
		...
		case SLIDING_MENU_STATE.CLOSING:
			var _current_x_pos = flexpanel_node_layout_get_position(ui_panel, false).left;
			var _new_x_pos = _current_x_pos + min(SLIDING_MENU_MOVEMENT_SPEED, view_w - _current_x_pos);
			flexpanel_node_style_set_position(ui_panel, flexpanel_edge.left, _new_x_pos, flexpanel_unit.point);
			...
			if(_new_x_pos >= view_w) {
				state = SLIDING_MENU_STATE.CLOSED;
			}
			break;
		case SLIDING_MENU_STATE.OPENING:
			var _current_x_pos = flexpanel_node_layout_get_position(ui_panel, false).left;
			var _new_x_pos = _current_x_pos + max(SLIDING_MENU_MOVEMENT_SPEED * -1, view_w - sprite_width - _current_x_pos);
			flexpanel_node_style_set_position(ui_panel, flexpanel_edge.left, _new_x_pos, flexpanel_unit.point);
			flexpanel_calculate_layout(ui_panel, camera_get_view_width(view_camera[0]), camera_get_view_height(view_camera[0]), flexpanel_direction.LTR);
			if(_new_x_pos <= view_w - sprite_width) {

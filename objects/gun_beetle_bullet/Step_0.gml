...
if(x >= 0 && x <= room_width && y >= 0 && y <= room_height) { //Only need to do boundary check if the bullet is actually in the room
	...
	collision_line_list(x, y, _end_x, _end_y, base_unit, false, true, entities_in_range, true);

	for(var i = 0; i < ds_list_size(entities_in_range); ++i) {
		if(entities_in_range[| i].health_state == HEALTH_STATE.ACTIVE) { //Don't want to shoot at knocked out units
			deal_damage(entities_in_range[| i], data.damage);
			_unit_was_hit = true;
		}
	}
